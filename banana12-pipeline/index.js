...
const { generateBanana12Prompt } = require('./claude');
const { generateGrid12Image } = require('./gemini');
const cloudinaryUtil = require('./cloudinary');
const { generateSlideshow, cleanup } = require('./slideshow');
...
async function processRecord(config, token, record) {
  ...
  const existingGrid12 = extractTextValue(fields.grid12);
  let gridUrl = existingGrid12;
  const timestamp = Date.now();

  if (!gridUrl) {
    // Step 1: Claude generates the Nano Banana 12-grid prompt
    const { nanoBananaPrompt } = await generateBanana12Prompt(
      config.anthropic,
      productDesc,
      productImgUrl,
      modelImgUrl
    ...
    // Step 2: Gemini generates the 12-grid image
    const { base64, mimeType } = await generateGrid12Image(
    ...
    const { secureUrl } = await cloudinaryUtil.uploadGrid12(base64, mimeType, timestamp, productId);
    gridUrl = secureUrl;
  } else {
    console.log(`[Info] grid12 already exists, skipping generation`);
  }

  // Step 4: Generate slideshow video from the grid image
  const { videoPath, tmpDir } = await generateSlideshow(gridUrl);

  // Step 5: Upload slideshow video to Cloudinary
  const { secureUrl: videoUrl } = await cloudinaryUtil.uploadVideo(videoPath, timestamp, productId);

  // Clean up temp files
  cleanup(videoPath, tmpDir);

  // Step 6: Update Feishu record
  await updateRecord(token, config.bitable.app_token, config.bitable.table_id, recordId, {
    grid12: gridUrl,
    slideshow12: videoUrl,
  });
  console.log(`[Done] Updated record ${recordId} grid12: ${gridUrl}`);
  console.log(`[Done] Updated record ${recordId} slideshow12: ${videoUrl}`);
}
